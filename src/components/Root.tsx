...
import ScreenshotExport from "./ScreenshotExport";
...
function Root() {
  ...
  const calculatedOutput = useMemo(() => {
    ...
    const calculationArgs = {
      ...
      tax: {
        amount: parseFloat(taxAmount ?? "0") / (taxUnit === "dollar" ? 1 : 100),
        unit: taxUnit,
      },
      tip: {
        amount: parseFloat(tipAmount ?? "0") / (tipUnit === "dollar" ? 1 : 100),
        unit: tipUnit,
      },
  ...
  return (
    <div className={"root"}>
      ...
      <div className="flex gap-4">
        <h1 className="text-4xl font-black">Gimme My Money</h1>
      ...
      <form className="mt-6 flex gap-4" onSubmit={handleAddParticipant}>
        ...
        <button
          ...
          disabled={newParticipantName.length === 0}
        >
          Add
        </button>
      </form>
      ...
      <div className="flex flex-row gap-2 flex-wrap mt-6">
        {participants.map((participant) => (
          <div className={"name-display"}>{participant}</div>
        ))}
      </div>
      ...
      <div className="mt-6">
        {items.map((item, i) => (
          <Fragment key={i}>
            ...
            <p className="mt-1.5 text-subtle text-[0.5875rem]">
              who bought this?
            </p>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!copy[i].participants.includes(participant))
                      ...
                      copy[i].participants = copy[i].participants.filter(
                        (p) => p !== participant,
                      );
      ...
      <button
        ...
        onClick={() =>
          setItems([...items, { name: "", price: "", participants: [] }])
        }
      >
        Add Item
      </button>
      ...
      <div className="flex gap-8 mt-6 justify-between">
        <div className="w-32">
          ...
          <CurrencyInput
            {...getCurrencyInputProps(tipUnit)}
            className="mt-1 input w-full text-right"
            value={tipAmount}
            onValueChange={(value) => {
              setTipAmount(value);
            }}
      ...
      <div className="mt-4">
        ...
        <div className="flex gap-2 mt-1 items-center">
          <input
            ...
            checked={tipOnTax}
          />
          <label htmlFor={"tip-on-tax"}>Tip included in tax</label>
        </div>
        ...
        <div className="flex gap-2 mt-1 items-center">
          <input
            ...
            checked={taxOnTip}
          />
          <label htmlFor={"tax-on-tip"}>Tax included in tip</label>
        </div>
      </div>
        ...
        <>
          <div className="mt-8 flex">
            ...
            <p className="total-price">
              ${calculatedOutput.subtotal.toFixed(2)}
            </p>
          </div>
          ...
          <button className="mt-4 button secondary" onClick={handleScreenshot}>
            Share Screenshot
          </button>
        </>
      )}
